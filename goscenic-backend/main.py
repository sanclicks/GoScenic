...
def calculate_trip_cost(origin: str, destination: str, mileage: float = 25.0):
    ...
    try:
        distance_meters = distance_response["rows"][0]["elements"][0]["distance"]["value"]
        distance_miles = distance_meters / 1609.34  # Convert meters to miles
    except KeyError:
        return {"error": "Unable to calculate distance. Check the origin and destination."}

    # 2. Get Gas Price (Using Default Value or Gas API)
    gas_price = 4.00  # Default value per gallon
    try:
        gas_price_url = f"https://api.gasbuddy.com/api/v1/gas-prices?api_key={GAS_API_KEY}"
        gas_response = requests.get(gas_price_url).json()
        gas_price = gas_response.get("average_gas_price", 4.00)  # Default to $4.00 if no response
    except:
    ...
    travel_days = max(1, distance_miles // 500)  # Assume 1 night stay per 500 miles
...
def download_itinerary(
    ...
    if format == "json":
        ...
        for restaurant in restaurant_data["restaurants"]:
            y -= 20
            c.drawString(120, y, f"- {restaurant['name']} ({restaurant['rating']}⭐)")

        c.save()

        return FileResponse(filename, media_type="application/pdf", filename=filename)

    return JSONResponse(content={"error": "Invalid format. Use 'json' or 'pdf'."}, status_code=400)

@app.get("/weather-along-route")
def get_weather_along_route(origin: str, destination: str):
    ...
    if "routes" not in directions_response or not directions_response["routes"]:
    ...
    weather_forecasts = []
    for waypoint in waypoints[:5]:  # Limit to 5 waypoints to optimize API usage
        weather_url = f"https://api.openweathermap.org/data/2.5/weather?lat={waypoint.split(',')[0]}&lon={waypoint.split(',')[1]}&appid={OPENWEATHER_API_KEY}&units=imperial"
        weather_response = requests.get(weather_url).json()

        weather_forecasts.append({
            "location": waypoint,
            "temperature": weather_response.get("main", {}).get("temp", "N/A"),
            "condition": weather_response.get("weather", [{}])[0].get("description", "N/A"),
...
def get_scenic_route(origin: str, destination: str):
    """Fetch scenic route with scenic places along the way."""

    # Step 1: Get multiple routes from Google Directions API
    directions_url = f"https://maps.googleapis.com/maps/api/directions/json?origin={origin}&destination={destination}&alternatives=true&key={GOOGLE_API_KEY}"
    directions_response = requests.get(directions_url).json()
    ...
    for route in directions_response["routes"]:
        ...
        scenic_places = []
        scenic_score = 0
        for waypoint in waypoints[:5]:  # Limit to 5 waypoints for optimization
            places_url = f"https://maps.googleapis.com/maps/api/place/nearbysearch/json?location={waypoint}&radius=10000&type=park|tourist_attraction|scenic_viewpoint|natural_feature&key={GOOGLE_API_KEY}"
            ...
            scenic_score += len(places_response.get("results", []))  # Score = number of scenic places

        # Step 4: Save route data
        scenic_routes.append({
            "route_summary": route["summary"],
            "distance_miles": round(route["legs"][0]["distance"]["value"] / 1609.34, 2),  # Convert meters to miles
            "duration_minutes": round(route["legs"][0]["duration"]["value"] / 60, 2),
    ...
    scenic_routes = sorted(scenic_routes, key=lambda x: x["scenic_score"], reverse=True)

    return {"recommended_scenic_route": scenic_routes[0], "all_routes": scenic_routes}
